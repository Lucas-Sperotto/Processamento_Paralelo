...
//****************************************************		
//		Código dividido em duas partes
//      Para divisão bidimensional alguns casos
//      ficaria da mesma forma que decompor unidimensionalmente
//		primeira parte cados unidimensionais
//****************************************************
		
if ((nProc != 4) && (nProc != 6) && (nProc != 8)){// para casos unidimensionais
		for (i = 0; i < nProc; i++){
			if (esteProc == i){
...
//*****************************************************
//		Para casos eu que se divide bidimensionalmente o domínio
//		
//*****************************************************
		else{
			iter = n;
				...
				if (esteProc == i){
					...
					//}
						...
						if (esteProc%2 ==0){
							//Troca Mensagens - atualiza zona fantasma
							//*******************************************************************************
							//		As trocas foram divididas para processos pares e impares, primeiro se envia os elementos das colunas
							//		depois as trocas das linhas e dos 'cantos', 
							//*******************************************************************************
							
							for(p = 1; p < (Vet[i]+1); p++){//envia elementos da coluna
							MPI_Send(&tabulOut[p][n], 1, MPI_INT, esteProc+1, p+esteProc+100, MPI_COMM_WORLD);
							}
							for(p = 1; p < (Vet[i]+1); p++){//recebe elementos da coluna
