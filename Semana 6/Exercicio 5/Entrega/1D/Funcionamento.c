...
#include "mpi.h"
	...
	int main(int argc, char* argv[]){
		...
		Divide(n, nProc, Vet);// Divide domionio e distribui o resto
		//for(i=0; i < nProc;i++)
			//printf("Vet[%d]=%d\n",i,Vet[i]);
		//}
		for (i = 0; i < nProc; i++){
			if (esteProc == i){// passa por todos os processadores e garante axecução para cada um, trabalhar com indice i
				...
				if (esteProc == 0){
					...
					for( p = 1; p < nProc; p++){
						...
						if (correto == 1) // Mestre soma as correções dos escravos
							c++;			
					}
					if ((c == (nProc-1)) && (Correto(Vet[i], n, tabulIn)) == 1){					
						printf("Resultado Correto\n");// se o mestre correto e o s escravos também
					}				
					else{
						printf("Resultado errado\n");
					}
				}
				else{// escravos mandam por mensagem sua correção
